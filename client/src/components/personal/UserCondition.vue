    ...
    <div class="display" v-for="item in conditionList">
      <el-card shadow="hover" style="height: 100%;">
        ...
        <div class="message" v-if="item.type === '音乐文章'|| item.type === '评论'">
          <div style="font-size: 14px; font-weight: bold; color: #78b6f7; margin-bottom: 5px">
            {{item.conditionMessage.title}}
          </div>
        ...
        <div class="message" v-if="item.type === '用户'|| item.type === '专栏'">
          <el-row :gutter="20" style="margin-top: 20px">
            ...
            <el-col :span="19">
              <div style="margin-top: 5px; font-size: 14px; cursor: pointer"
                   @click="displayUserInfo(item.conditionMessage.name)" v-if="item.type === '用户'">
                {{item.conditionMessage.name}}
              </div>
              <div style="margin-top: 5px; font-size: 14px; cursor: pointer"
                   @click="displayTopicInfo(item.conditionMessage.id)" v-if="item.type === '专栏'">
                {{item.conditionMessage.name}}
              </div>
              <div style="margin-top: 5px; font-size: 12px">{{item.conditionMessage.introduce}}</div>
              <div style="margin-top: 20px; font-size: 10px; color: #969696"><span>{{item.conditionMessage.productionNum}}作品 </span>
                <span style="margin-left: 15px"> {{item.conditionMessage.attentionNum}}人关注</span></div>
            </el-col>
            <el-col :span="2" style="margin-top: 20px;">
              <el-button v-if="item.conditionMessage.status" @click="cancelAttention(item.conditionMessage.name)"
                         style="width: 80px; height: 40px">已 关 注
              </el-button>
              <el-button v-else @click="attention(item.conditionMessage.name)" style="width: 80px; height: 40px">关 注
              </el-button>
            </el-col>
          </el-row>
        </div>
