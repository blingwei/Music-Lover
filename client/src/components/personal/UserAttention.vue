      ...
      <el-tab-pane label="我的关注人" name="myAttention">
          <div class="info" v-for="item in myAttentionList">
              <el-row :gutter="20">
                  ...
                  <el-col :span="19">
                      <div style="margin-top: 5px; font-size: 14px" @click="displayUserInfo(item.name)">{{item.name}}</div>
                      <div style="margin-top: 5px; font-size: 12px">{{item.introduce}}</div>
                      <div style="margin-top: 20px; font-size: 10px; color: #969696"><span>{{item.productionNum}}作品 </span> <span style="margin-left: 15px"> {{item.attentionNum}}人关注</span></div>
                  </el-col>
                  <el-col :span="2" style="margin-top: 20px;">
                      <el-button v-if="item.status" @click="cancelAttention(item.name)" style="width: 80px; height: 40px">已 关 注</el-button>
      ...
      <el-tab-pane label="关注的专栏" name="attentionTopic">
          <div class="info" v-for="item in attentionTopicList">
              <el-row :gutter="20">
                  ...
                  <el-col :span="19">
                      <div style="margin-top: 5px; font-size: 14px" @click="displayTopicInfo(item.id)">{{item.name}}</div>
                      <div style="margin-top: 5px; font-size: 12px">{{item.introduce}}</div>
                      <div style="margin-top: 20px; font-size: 10px; color: #969696"><span>{{item.productionNum}}作品 </span> <span style="margin-left: 15px"> {{item.attentionNum}}人关注</span></div>
                  </el-col>
                  <el-col :span="2" style="margin-top: 20px;">
                      <el-button v-if="item.status" @click="cancelAttentionTopic(item.id)" style="width: 80px; height: 40px">已 关 注</el-button>
